# Generative AI Big Picture

Workshop materials and resources for understanding generative AI, large language models, and practical applications.

## Presentations

- [Generative AI Holistic Perspective](https://docs.google.com/presentation/d/1LaPTo8OaWBcuuTXnNSo6dITLKxsvA6s5/edit?usp=sharing&ouid=109036277148056682596&rtpof=true&sd=true)
- [Customizing LLMs for Your Use Case](https://docs.google.com/presentation/d/1MBiJQxxCbP-6eCPzxiuoCXbAV3Pj9vnn/edit?usp=sharing&ouid=109036277148056682596&rtpof=true&sd=true)
- [Prompt Engineering](https://docs.google.com/presentation/d/1MBiJQxxCbP-6eCPzxiuoCXbAV3Pj9vnn/edit?usp=sharing&ouid=109036277148056682596&rtpof=true&sd=true)
- [AI Ethics and Safety](https://docs.google.com/presentation/d/12ESFnEif24kPsxPfv1dGOszgt2eab6Ab/edit?usp=sharing&ouid=109036277148056682596&rtpof=true&sd=true)

## Repository Structure

```
├── MARKDOWN/          # Written guides and documentation
│   ├── 01-big-picture-overview.md
│   ├── 02-getting-the-most-from-genai.md
│   ├── 03-practice-getting-the-most.md
│   ├── 04-Bias Examples.md
│   └── real_world_case_studies.md
│
├── NOTEBOOKS/         # Jupyter notebooks for hands-on exercises
│   ├── Chatbot_with_Groq.ipynb
│   ├── DocuSign_Agreement_AI_Agent.ipynb
│   └── HKS_LAB_Prompt_Engineering_with_Groq.ipynb
│
├── NOTES/             # PDF reference materials
│
└── PROJECT WORK/      # Workshop exercises and AI agent examples
    ├── Groq_Hands_On_Workshop.ipynb
    └── AI-AGENT 2/    # DocuSign AI agent implementation
```

## Getting Started

1. Review the presentations above for conceptual foundations
2. Explore the markdown guides in `MARKDOWN/` for detailed explanations
3. Work through the Jupyter notebooks in `NOTEBOOKS/` for hands-on practice
4. Check `PROJECT WORK/` for workshop exercises and agent implementations

## Topics Covered

- Introduction to Generative AI
- Understanding Groq and Llama models
- Prompt engineering techniques
- Real-world applications and case studies
- AI ethics, bias, and safety considerations
- Building AI agents with DocuSign integration
